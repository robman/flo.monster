...
// Set at build time via esbuild define, or at runtime via configure_api_base message.
declare const __API_BASE_URL__: string;
let apiBaseUrl: string = typeof __API_BASE_URL__ !== 'undefined' ? __API_BASE_URL__ : '';
...
const configLoaded: Promise<void> = loadConfig();
...
self.addEventListener('message', (event: ExtendableMessageEvent) => {
  ...
  if (event.source && 'type' in event.source && (event.source as { type: string }).type !== 'window') {
      ...
      if (Array.isArray(data.urls)) {
        event.waitUntil(
          caches.open(SHELL_CACHE).then(cache =>
            Promise.all(data.urls.map((url: string) =>
...
// Build-time injected list of hashed asset URLs (JS + CSS from dist/assets/).
// Replaced by esbuild define in build-sw.js; defaults to empty in dev/test.
declare const __PRECACHE_ASSETS__: string[];
const BUILD_ASSETS: string[] = typeof __PRECACHE_ASSETS__ !== 'undefined' ? __PRECACHE_ASSETS__ : [];

self.addEventListener('install', (event: ExtendableEvent) => {
  // BUILD_ASSETS contains all dist/ files (injected at build time).
  // '/' is always included (the HTML entry point, served as index.html).
